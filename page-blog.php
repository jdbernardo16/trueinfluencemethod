 ...
 * Description: Blogs & Podcast landing page template
 *
 * @package tim-wordpress
 */

// Get latest episodes/posts
...
$posts = new WP_Query([
    ...
    'orderby' => 'date',
    'order' => 'DESC',
    'post__not_in' => wp_list_pluck($latest_posts->posts, 'ID'),
]);
...
<body <?php body_class(); ?>>
    ...
    <div class="overflow-x-hidden">
            ...
            <div class="max-w-5xl mx-auto px-6 relative z-10 text-center">
                <div class="mb-8">
                    <span class="inline-flex items-center gap-2 bg-[#d4b478]/10 border border-[#d4b478]/30 text-[#d4b478] text-xs font-bold tracking-[0.2em] uppercase px-4 py-2 rounded-full">
                        ...
                        <?php echo esc_html(tim_get_field('blog_hero_badge', 'Latest Episodes')); ?>
                    </span>
                </div>

                <h1 class="font-serif text-4xl md:text-6xl lg:text-7xl text-[#faf8f5] mb-8 leading-tight">
                    <?php echo esc_html(tim_get_field('blog_hero_heading', 'Conversations That Matter')); ?>
                </h1>

                <p class="text-[#faf8f5]/80 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed mb-10">
                    <?php echo esc_html(tim_get_field('blog_hero_description', 'Join Joanna for honest conversations about leadership, authenticity, and the journey to finding your voice. New episodes every week.')); ?>
                </p>

                <?php
                $cta_link = tim_get_field('blog_hero_cta_link', '');
                $cta_url = $cta_link ? get_permalink($cta_link) : '#blog-grid';
                ?>
                <a href="<?php echo esc_url($cta_url); ?>"
                    ...
                    <?php echo esc_html(tim_get_field('blog_hero_cta_text', 'Listen Now')); ?>
                ...
                <div class="max-w-7xl mx-auto px-6 relative z-10">
                    <div class="text-center mb-16">
                        <span class="inline-flex items-center gap-2 bg-[#d4b478]/10 border border-[#d4b478]/30 text-[#d4b478] text-xs font-bold tracking-[0.2em] uppercase px-4 py-2 rounded-full mb-6">
                            <?php echo esc_html(tim_get_field('blog_latest_badge', 'Fresh Content')); ?>
                        </span>
                        <h2 class="font-serif text-3xl md:text-5xl text-[#0f203d] mb-6">
                            <?php echo esc_html(tim_get_field('blog_latest_heading', 'Latest Episodes')); ?>
                        </h2>
                        <p class="text-[#0f203d]/70 text-lg max-w-2xl mx-auto">
                            <?php echo esc_html(tim_get_field('blog_latest_description', 'Catch up on the most recent conversations and insights from Joanna.')); ?>
                        </p>
                    </div>
        ...
        <section id="blog-grid" class="py-20 md:py-32 bg-[#0f203d] text-[#faf8f5] relative overflow-hidden">
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#d4b478]/5 to-transparent"></div>

            <div class="max-w-7xl mx-auto px-6 relative z-10">
                <div class="text-center mb-16">
                    <span class="inline-flex items-center gap-2 bg-[#d4b478]/10 border border-[#d4b478]/30 text-[#d4b478] text-xs font-bold tracking-[0.2em] uppercase px-4 py-2 rounded-full mb-6">
                        <?php echo esc_html(tim_get_field('blog_grid_badge', 'Archive')); ?>
                    </span>
                    <h2 class="font-serif text-3xl md:text-5xl text-[#faf8f5] mb-6">
                        <?php echo esc_html(tim_get_field('blog_grid_heading', 'All Episodes')); ?>
                    </h2>
                    <p class="text-[#faf8f5]/70 text-lg max-w-2xl mx-auto">
                        <?php echo esc_html(tim_get_field('blog_grid_description', 'Browse the complete archive of conversations and insights.')); ?>
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <?php else : ?>
                        <div class="col-span-full text-center py-20">
                            <p class="text-[#faf8f5]/60 text-lg">No episodes found.</p>
                        </div>
                    <?php endif; ?>
                </div>
